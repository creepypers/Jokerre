...
export const ProjectDetailsScreen: React.FC<ProjectDetailsScreenProps> = ({
  ...
  const allTickets = getTicketsByProject(project.id);
  ...
  const handleEditTicket = (ticket: Ticket) => {
    ...
    const currentAssignee = projectMembers.find(
      (member) => member.id === ticket.assignee
    );
    setSelectedAssignee(
      currentAssignee
        ? {
  ...
  const getAssigneeName = (assigneeId?: string) => {
    ...
    return (
      projectMembers.find((m) => m.id === assigneeId)?.displayName || "Inconnu"
    );
  };
          ...
          <View style={styles.ticketInfo}>
            ...
            <View
              style={[
                ...
                {
                  backgroundColor:
                    PRIORITY_CONFIG[ticket.priority].color + "20",
                },
              ]}
            >
              <Text
            ...
            {statusConfig.prevStatus && statusConfig.prevLabel && (
              <TouchableOpacity
                ...
                onPress={() =>
                  handleTicketMove(
                    ticket,
                    statusConfig.prevStatus as "todo" | "in-progress" | "done"
                  )
                }
              >
                <Text style={styles.moveButtonText}>
                  {statusConfig.prevLabel}
                </Text>
              </TouchableOpacity>
            )}
            {statusConfig.nextStatus && statusConfig.nextLabel && (
              <TouchableOpacity
                ...
                onPress={() =>
                  handleTicketMove(
                    ticket,
                    statusConfig.nextStatus as "todo" | "in-progress" | "done"
                  )
                }
              >
                <Text style={styles.moveButtonText}>
      ...
      <View style={styles.pageContainer}>
        <View style={styles.pageTitleContainer}>
          <Text style={styles.pageTitle}>
            Tickets {statusConfig.label.toLowerCase()}
          </Text>
          <CompactFilters
            filters={[
              {
                ...
                options: [
                  { value: "all", label: "Tous", count: allTickets.length },
                  {
                    value: "unassigned",
                    label: "Non assigné",
                    count: allTickets.filter((t) => !t.assignee).length,
                  },
                  ...projectMembers.map((member) => ({
                    value: member.id,
                    label: member.displayName || member.email,
                    count: allTickets.filter((t) => t.assignee === member.id)
                      .length,
                  })),
                ],
            ...
            style={styles.compactFiltersInline}
          />
        </View>
        <FlatList
          data={tickets}
        ...
        rightElement={
          <View style={styles.headerActions}>
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate("TeamManagement", { projectId: project.id })
              }
            >
              <Icon source="account-group" size={16} color="white" />
            </TouchableOpacity>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.tabContent}>
          ...
          {activeTab === "in-progress" &&
            renderTabContent("in-progress", inProgressTickets)}
            ...
            {Object.entries(PRIORITY_CONFIG).map(([key, config]) => (
              <View key={key} style={styles.priorityCheckboxItem}>
                <Checkbox
                  ...
                  onPress={() =>
                    setTicketPriority(
                      key as "low" | "medium" | "high" | "urgent"
                    )
                  }
                  color={config.color}
                />
                <Text
                  style={[styles.priorityCheckboxText, { color: config.color }]}
                >
                  {config.label}
                </Text>
              </View>
